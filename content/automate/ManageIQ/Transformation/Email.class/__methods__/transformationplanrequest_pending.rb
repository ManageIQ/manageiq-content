...
def approver_text(appliance, requester_email)
  body = "Approver, "
  body += "<br>A Transformation Plan request received from #{requester_email} is pending."
...
def email_approver(appliance)
  ...
  subject = "Request ID #{@miq_request.id} - Transformation Plan request was not approved"
...
def email_requester(appliance)
  ...
  from = $evm.object['from_email_address']
  subject = "Request ID #{@miq_request.id} - Your Transformation Plan Request was not Auto-Approved"
