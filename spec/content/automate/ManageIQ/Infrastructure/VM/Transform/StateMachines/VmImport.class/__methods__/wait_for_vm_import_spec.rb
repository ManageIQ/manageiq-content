...
describe ManageIQ::Automate::Infrastructure::VM::Transform::StateMachines::WaitForVmImport do
  ...
  context 'On missing import status' do
    let!(:vm) { FactoryBot.create(:vm_redhat, :ems_ref => new_ems_ref) }
  ...
  context 'On successful import status' do
    let!(:custom_attribute) { FactoryBot.create(:miq_custom_attribute, :name => 'import_status', :value => 'success') }
    let!(:vm)               { FactoryBot.create(:vm_redhat, :ems_ref => new_ems_ref, :custom_attributes => [custom_attribute]) }
  ...
  context 'On failed import status' do
    let!(:custom_attribute) { FactoryBot.create(:miq_custom_attribute, :name => 'import_status', :value => 'failure') }
    let!(:vm)               { FactoryBot.create(:vm_redhat, :ems_ref => new_ems_ref, :custom_attributes => [custom_attribute]) }
