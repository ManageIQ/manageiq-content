...
describe ManageIQ::Automate::Cloud::VM::Retirement::StateMachines::CheckPreRetirement do
  ...
  context "vm power state" do
    ...
    let(:svc_model_vm) { MiqAeMethodService::MiqAeServiceVmOrTemplate.find(vm.id) }
    let(:root_hash) do
      {
        'vm' => MiqAeMethodService::MiqAeServiceVmOrTemplate.find(vm.id)
      }
    end
    ...
    it "never" do
      svc_model_vm
      described_class.new(ae_service).main
      expect(ae_service.root['vm'].raw_power_state).to eq('never')
