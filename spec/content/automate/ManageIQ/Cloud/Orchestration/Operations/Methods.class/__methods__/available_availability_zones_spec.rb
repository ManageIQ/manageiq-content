...
describe ManageIQ::Automate::Cloud::Orchestration::Operations::AvailableAvailabilityZones do
  ...
  shared_examples_for "orchestration manager" do
    context "with a single availability zone" do
      let(:az1) { FactoryBot.create(:availability_zone, :ems_ref => "ref1") }
      let(:ems) { FactoryBot.create(:ems_vmware_cloud, :availability_zones => [az1]) }
    ...
    context "with multiple availability zones" do
      let(:az2) { FactoryBot.create(:availability_zone, :ems_ref => "ref2") }
      let(:az3) { FactoryBot.create(:availability_zone, :ems_ref => "ref3") }
      let(:ems) { FactoryBot.create(:ems_vmware_cloud, :availability_zones => [az2, az3]) }
